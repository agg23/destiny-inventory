...
describe("absorb", () => {
  ...
  it("stamps the moment an order first reads complete", () => {
    ...
    const done = absorb(first, [order("a", { complete: true })], NOW + 5_000);
  ...
  it("does not log an expired order that never completed", () => {
    ...
    const gone = absorb(absorb(EMPTY_LEDGER, [stale], NOW), [], NOW + 1_000);

    expect(gone.log).toEqual([]);
  });

  it("logs newest first and leaves earlier entries alone", () => {
    let ledger = absorb(EMPTY_LEDGER, [order("a", { complete: true })], NOW);
