...
import { getTestDefinitions, getTestProfile, testAccount as rawTestAccount } from './test-utils.ts';
...
export function placeItemInPostmaster(
  ...
  const lostItems = buckets.byHash[BucketHashes.LostItems];

  if (!lostItems) {
    throw new Error("Manifest has no Lost Items bucket");
  }

  const relocated: DimItem = { ...item, location: lostItems, canPullFromPostmaster: true };
  return [
